...
val ys = xs.toSeq
\end{Code}
\Subtask Hur fungerar likhetstest mellan två \code{ArraySeq}s? Vad har \code{ys} ovan för typ? Går det att uppdatera en wrappad array?
...
\SubtaskSolved \code{ys} erbjuder innehållslikhet och har typen \code{Seq[Int]} med den underliggande typen \code{ArraySeq[Int]}. Det går inte att göra tilldelning av element i en \code{ArraySeq} eftersom metoden \code{update} saknas, och den är oföränderlig. Den uppdateras därför inte när den urspringliga arrayen uppdateras.

\begin{REPL}
scala> val xs1 = Array(1,2,3)
val xs1: Array[Int] = Array(1, 2, 3)

scala> val xs2 = Array(1,2,3)
val xs2: Array[Int] = Array(1, 2, 3)


scala> val (ys, ys) = (xs1.toSeq, xs2.toSeq)
val ys: Seq[Int] = ArraySeq(1, 2, 3)
val ys: Seq[Int] = ArraySeq(1, 2, 3)
...
\begin{REPL}
scala> def f(ys: Int*) = ys
def f(ys: Int*): Seq[Int]
